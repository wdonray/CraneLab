...
namespace Combu
{
	...
	public class CombuManager : MonoBehaviour
	{
		...
		/// Current API version.
		/// </summary>
		public const string COMBU_VERSION = "3.1.0";
				...
				if (!string.IsNullOrEmpty(token) && (!string.IsNullOrEmpty(xml) || (!string.IsNullOrEmpty(modulus) && !string.IsNullOrEmpty(exponent))))
				{
					...
					CallWebservice(GetUrl("server.php"), form, (string textAuthorize, string errorAuthorize) => {
						...
						if (!successAuthorize) {
							...
							/*var cbForm = CreateCombuForm();
							cbForm.AddField("a", "testA");
							cbForm.AddField("b", new Hashtable(){{"x","y"}}.toJson());
							CallWebservice(GetUrl("test.php"), cbForm, (string tx, string er) => {
								Debug.LogWarning(tx);
							});*/
							// Get the server info
